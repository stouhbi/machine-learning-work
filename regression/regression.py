...
with tf.Session() as sess:
    ...
    for iteration in range(training_iteration):
        ...
        if iteration % display_step == 0:
            training_cost = sess.run(cost_function, feed_dict={X:size_data_n, Y:price_data_n})
            print "Iteration:", '%04d' % (iteration + 1), "cost=", training_cost,\
            "W=", sess.run(W), "b=", sess.run(b)

    tuning_cost = sess.run(cost_function, feed_dict={X: normalize(size_data_n), Y: normalize(price_data_n)})

    print "Tuning completed:", "cost=", tuning_cost, "W=", sess.run(W), "b=", sess.run(b)
